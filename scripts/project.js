document.addEventListener('DOMContentLoaded', function () {
    // Set last modified date and copyright year
    document.getElementById('last-modified').textContent = document.lastModified;
    document.getElementById('copyright-year').textContent =
        new Date().getFullYear();

    // Show current date and time and keep updating every second
    function updateCurrentDateTime() {
        const element = document.getElementById('current-date-time');
        if (!element) return;
        const now = new Date();
        element.textContent = `Current Date & Time: ${now.toLocaleString()}`;
    }
    updateCurrentDateTime();
    setInterval(updateCurrentDateTime, 1000);

    // Navigation menu toggle handling
...
function displayMembers(members) {
    ...
    members.forEach((member) => {
        ...
        memberCard.innerHTML = `
            <img data-src="${member.image}" loading="lazy" alt="${member.name} logo" style="width: 80px; height: 80px; object-fit: contain;">
            <h3>${member.name}</h3>
            <p>${member.address}</p>
            <p>${member.phone}</p>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (messageElem) {
        if (!lastVisit) {
            // First visit
            messageElem.innerText = "Welcome! Let us know if you have any questions.";
        } else {
            ...
            if (daysDiff < 1) {
                messageElem.innerText = "Back so soon! Awesome!";
            } else {
                let daysText = daysDiff === 1 ? 'day' : 'days';
                messageElem.innerText = `You last visited ${daysDiff} ${daysText} ago.`;
            }
        }
    }
